#!/usr/bin/env python3
"""Count employees in March 2025 OPL sheet - from 'employees' column"""
...
def main():
    ...
    print("="*120)
    print("MARCH 2025 - COUNTING EMPLOYEES FROM 'EMPLOYEES' COLUMN IN OPL SHEET")
    print("="*120)
    ...
    try:
        ...
        for col_idx, header in enumerate(headers):
            ...
            # Match exactly "employee" but not "employee id"
            if h == 'employee':
                employees_col = col_idx
                print(f"  Found Employee column at index {col_idx}: '{header}'")
                break

        # If not found, try "employees" (plural)
        if employees_col is None:
            for col_idx, header in enumerate(headers):
                h = str(header).lower().strip()
                if h == 'employees':
                    employees_col = col_idx
                    print(f"  Found Employees column at index {col_idx}: '{header}'")
                    break
        ...
        # Count entries in employees column
        count = 0
        ...
        print(f"\n  Total employees in 'employees' column: {count}")

        print(f"\n{'='*120}")
        print(f"TOTAL OPL EMPLOYEES IN MARCH 2025: {count}")
        print(f"{'='*120}")

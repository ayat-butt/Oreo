#!/usr/bin/env python3
"""
CORRECTED: Complete OPL Audit July 2024 - June 2025
Properly identifies OPL employees by "Employee Grade" = "Orenda Private Limited"
"""
...
def extract_opl_from_excel(drive_service, file_id, month_name):
    """Download and extract OPL employees from Excel"""
    try:
        ...
        for idx, row in df.iterrows():
            ...
            # Check if this is an OPL employee
            if entity_col and pd.notna(row[entity_col]):
                if 'Orenda Private Limited' not in str(row[entity_col]):
                    continue
...
def main():
    ...
    print("="*120)
    print("COMPLETE OPL AUDIT: JULY 2024 - JUNE 2025 (CORRECTED)")
    print("="*120)
    ...
    all_sheets = {
        'July 2024': '1NLyPSfEO7N92EjGIBy6EnrrjvWmE7QTi',
    ...
    # JULY - OCTOBER: Excel files
    print("Reading July - October 2024 (Excel files):")
    for month in ['July 2024', 'August 2024', 'September 2024', 'October 2024']:
        sheet_id = all_sheets[month]
        employees = extract_opl_from_excel(drive_service, sheet_id, month)
        monthly_data[month] = employees
        print(f"  {month}: {len(employees)} OPL employees")

    # NOVEMBER 2024 - JUNE 2025: Google Sheets
    ...
    try:
        create_request = {
            'properties': {
                'title': 'OPL Joiners & Leavers Jul24-Jun25 (FINAL CORRECTED)'
            }
        }
        ...
        print("="*120)
        print("FINAL CORRECTED AUDIT SHEET CREATED")
        print("="*120)

#!/usr/bin/env python3
"""Export ALL employees and dependents with NO filters."""
...
def export_employees_csv():
    """Export ALL CURRENT employee profiles to CSV."""
    print("Fetching ALL current employees...")
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor(cursor_factory=RealDictCursor)

    # Get ONLY current employees (with CNIC - they have filled in profile)
    cur.execute("""
        SELECT
            user_id,
            employee_id,
    ...
    # Write to CSV
    output_file = "c:\\Agent Oreo\\output\\EMPLOYEES_PROFILES_COMPLETE.csv"
    ...
    print(f"[OK] Exported {len(employees)} employees")
    return len(employees)

def export_all_dependents_csv():
    """Export ALL DEPENDENTS (123 including the missing one)."""
    print("Fetching ALL dependents...")
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor(cursor_factory=RealDictCursor)

    # Get ALL dependents for current employees
    cur.execute("""
        SELECT
            ep.user_id,
            ep.employee_id,
    ...
    # Write to CSV
    output_file = "c:\\Agent Oreo\\output\\EMPLOYEE_DEPENDENTS_COMPLETE.csv"
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
    ...
    print(f"[OK] Exported {len(dependents)} dependents")
    return len(dependents)

def main():
    print("\n" + "=" * 80)
    print("EXPORTING COMPLETE EMPLOYEE & DEPENDENT DATA")
    print("=" * 80)
    print()

    emp_count = export_employees_csv()
    dep_count = export_all_dependents_csv()
    ...
    print(f"[OK] Total Dependents: {dep_count} (INCLUDING THE MISSING ONE)")
    print()
    print("Files created:")
    print("  1. output/EMPLOYEES_PROFILES_COMPLETE.csv")
    print("  2. output/EMPLOYEE_DEPENDENTS_COMPLETE.csv")
